...
def collect_signatures(
    ...
    signature_summary = ", ".join(
        f"{group['signature']} ({group['alert_count']})" for group in signature_groups
    )
    logger.info(
...
def analyze_signatures(
    ...
    def get_connection_payload(conn_id: str) -> tuple[object, dict[str, object]]:
        conn = vault.get(conn_id)
        return conn, conn.extra
    ...
    def resolve_network_identity(ip: str | None, observed_at: str) -> dict[str, object]:
        if not ip:
            return {
                "ip": None,
                "mac_address": None,
    ...
    def analyze_one_alert(
        ...
        finding = llm.with_structured_output(AlertFinding).invoke(prompt).model_dump()
        ...
        finding["asset_identity"] = " / ".join(source_identity_parts) or "unresolved"
    ...
    with tracer.start_as_current_span("cyber_analyst.analyze_signatures"):
        for signature_group in signature_groups:
            with tracer.start_as_current_span("cyber_analyst.signature_agent") as span:
                ...
                analysis = signature_agent.invoke({"signature_group": signature_group})
                logger.info(
